...
def register(request):
    ...
    if not request.POST.get('zen'):
        messages.error(request,'请认真阅读《Pythonic社区指导原则》。')
    ...
    try:
        ...
        messages.error(request,'此名号已被使用')
        return render(request,'register.html',locals())
    # 创建新用户
    ...
    try:
        ...
        new_profile = UserProfile(user=new_user,name=data['name'],slug=data['name'],city=data['city'])
        pass
    else:
        ...
        aaa = urlretrieve(gravatar_url,res_path)
        new_profile = UserProfile(user=new_user,name=data['name'],slug=data['name'],city=data['city'],avatar=res_name,photo=res_name2)
    try:
        new_user.save()
        new_profile.save()
...
#         )
        ...
        active_url = '%s/accounts/active/%d/%s/' %('http://'+DOMAIN,user.id,_get_active_code(user.username))
...
def crop_avatar(request):
    """剪裁头像"""
#     photo_orig = UserProfile.objects.get(user=request.user).get_photo_path()
#     if not photo_orig:
